  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _renderEvent: function(calEvent, $weekDay) {
          ...
          if (!options.readonly && options.draggable(calEvent, $calEventList)) {
            self._addDraggableToCalEvent(calEvent, $calEventList, $weekDay);
          }
      ...
      _adjustForEventCollisions: function($weekDay, $calEvent, newCalEvent, oldCalEvent, maintainEventDuration) {
          ...
          $weekDay.find('.wc-cal-event').not($calEvent).each(function() {
            ...
            if (currentCalEvent && newCalEvent.start.getTime() < currentCalEvent.end.getTime() &&
            ...
            if (currentCalEvent && newCalEvent.end.getTime() > currentCalEvent.start.getTime() &&
            ...
            if (oldCalEvent.resizable == false ||
                  (currentCalEvent && newCalEvent.end.getTime() <= currentCalEvent.end.getTime() &&
                    newCalEvent.start.getTime() >= currentCalEvent.start.getTime())) {
       ...
       */
      _addDraggableToCalEvent: function(calEvent, $calEvent, $weekDay) {
        var self = this;
        var options = this.options;
        ...
        $calEvent.draggable({
          ...
          drag: function(event, ui) {
            ...
            $calEvent.find('.wc-time').html(self.options.eventHeader(newCalEvent, self.element));
          }
        });
      },
